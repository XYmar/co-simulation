...
public class SubtaskService {
    ...
    @Autowired
    public SubtaskService(SubtaskRepository subtaskRepository, ProjectService projectService, UserService userService, AllIllustrationRepository allIllustrationRepository) {
...
//        subtaskEntity.setPassState(0);
//        subtaskEntity.setCountersignState(0);
//        subtaskEntity.setReject(ASSESS_STATE_PASS);
//        subtaskEntity.setOperationState(0);
...
//    }
    ...
    public SubtaskEntity assessSubtaskByIds(String subtaskById, SubtaskEntity subtaskEntityArgs, UserEntity userEntity) {
        ...
        if (!CollectionUtils.isEmpty(allIllustrationList)) {
            if (subtaskEntityArgs.getState() - maxState > 3 && subtaskEntityArgs.getCountersignState() != 0) {
        ...
        if (subtaskEntity.getCountersignState() == ApplicationConfig.MANY_PEOPLE_COUNTERSIGNSTATE && subtaskEntityArgs.getState() == ApplicationConfig.COUNTERSIGN_ING) {
            if (!subtaskEntityOptional.isPresent()) {
                throw new ResultException(ResultCode.PRODESIGN_LINK_ARGS_NOT_FOUND_ERROR);
            }
